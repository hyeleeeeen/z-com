...
export default function ActionButtons({ white, post }: Props) {
  ...
  const liked = !!post.Hearts?.find((v) => v.userId === session?.user?.email); // 현재 계정이 좋아요 눌렀는지 확인
  ...
  const unheart = useMutation({
    ...
    onError() {
      const queryCache = queryClient.getQueryCache();
      const queryKeys = queryCache.getAll().map((cache) => cache.queryKey); // 모든 쿼리키 불러오기
      console.log("queryKeys", queryKeys);
      queryKeys.forEach((queryKey) => {
        if (queryKey[0] === "posts") {
          console.log(queryKey[0]);
    ...
    onSettled() {
      // queryClient.invalidateQueries({ queryKey: ["posts"] }); // 'post'로 시작하는 쿼리키 상태 업데이트 (서버와 상태 일치하기위해)
    },
  });
