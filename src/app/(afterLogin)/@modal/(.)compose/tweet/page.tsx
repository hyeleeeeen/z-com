...
export default function TweetModal() {
  ...
  const parent = modalStore.data; // 답글 달 원 게시물
  ...
  const onSubmit: FormEventHandler<HTMLFormElement> = (e) => {
    if (modalStore.mode === "new") {
      // 분기처리
      mutation.mutate(e); // 새글작성
    } else {
      comment.mutate(e); // 댓글작성
    }
  };
  const onClickClose = () => {
    router.back();
  ...
  return (
    <div className={style.modalBackground}>
      <div className={style.modal}>
        <button onClick={onClickClose} className={style.closeButton}>
          {" "}
          <svg
            width={24}
            viewBox="0 0 24 24"
          ...
          {modalStore.mode === "comment" && parent && (
            <div className={style.modalOriginal}>
              ...
              <div>
                ...
                <div>
                  <Link href={`/${parent.User.id}`} className={style.parentId}>
                    @{parent.User.id}
                  </Link>
                  님에게 보내는 달글
                </div>
